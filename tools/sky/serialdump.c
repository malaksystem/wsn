...
#define MODE_READ_FILE   9


#define READ_STAR_NUM   60

static unsigned char rxbuf[2048];

bool is_read_file(unsigned char in);
    ...
    if(FD_ISSET(fd, &smask)) {
      ...
      for(i = 0; i < n; i++) {
        switch(mode) {
          ...
          case MODE_TEXT:
            printf("%c", buf[i]);
            if(is_read_file(buf[i])){
              raw_fp = fopen("/home/dd/raw_data", "a+");
              if(raw_fp == NULL){
...
bool 
is_read_file(unsigned char in)
{
  static int star_num = 0;

  if(in == '*'){
    if(++star_num == READ_STAR_NUM){
      return true;
    }
    else{
      return false;
